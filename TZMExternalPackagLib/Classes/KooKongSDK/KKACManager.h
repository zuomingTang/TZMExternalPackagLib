...
/**
 初始化KKACManager类使用该方法
 
 @param remoteid 遥控器id
 @param irData 码库数据
 @param array 用户上次使用空调保存的各种模式及模式下的状态值（从getACAllModeAndStateValue方法中获取，如果该参数传nil，各个模式状态值为默认初始值，该参数取代了readAirConditionStateAndValueWihtModestate方法）
 @return KKACManager对象
 */
- (instancetype)initWithRemoteId:(NSString *)remoteid irData:(NSDictionary *)irData modeStateValue:(NSArray *)array;


/**
 *  使用构造函数初始Manager的，不需要调用该方法
 *  处理红外码库数据
 */
...
/**
 *  非首次使用时，将上次使用后保存的空调的所有模式及模式下的状态值传值
 *
 *  @param modesta  模式
 *  @param powersta 开关
 *  @param temp     温度
 ...
 *  @param windp 风量（在接口：-(NSArray *)getAllWindPower;取到所需传入的参数）
 *  @param temp 温度（温度为16~30，在接口：-(NSArray *)getLackOfTemperatureArray;
 取到限制设定的温度值）
 *  @param powersta 开关（AC_POWER_ON：打开，AC_POWER_OFF：关闭）
 *  @param windsta 风向（在接口：-(NSArray *)getAllWindState;取到所需传入的参数）
 */
 ...
 *  @return 1501：制冷，1502：制热，1503：自动，1504：送风，1505：除湿
 */
-(NSArray *)getAllModeState;


/**
 *  取到当前模式下的所有的风量(若返回nil，表示风量不可控)
 *
 *  @return nil：表示风量不可控，0：自动风量，1：低风量，2：中风量，3：高风量
 */
-(NSArray *)getAllWindPower;


/**
 *  取到空调支持的所有的风向（若返回nil，表示风向标不可控）
 *
 *  @return nil：表示风向不可控，0：扫风，！0：固定风向
 */
-(NSArray *)getAllWindState;

/**
 *  取到当前模式下限制设定的温度值集（若返回nil，表示无限制设定温度值）
 *
 *  @return 得到限制设定的温度集合，在改变温度时，要避免传入限制设定的温度值；没有限制设定的温度值返回：nil 
 */
-(NSArray *)getLackOfTemperatureArray;
 ...
 *  @return 将取到的值发送出去
 */
-(NSArray *)getAirConditionInfrared;
 ...
 *  @return 根据自身需求去处理这些值
 */
-(NSArray *)getAirConditionAllModeAndValue;
...
/**
 获取用户上一次使用空调时的各种模式以及对应模式下的状态值

 @return 空调支持的各种模式以及模式下对应的状态值
 */
- (NSArray *)getACAllModeAndStateValue;
